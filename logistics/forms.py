...
class RouteForm(forms.ModelForm):
    ...
    def clean(self):
        ...
        if from_location and to_location and from_location == to_location:
            raise ValidationError("Начальный и конечный пункты маршрута должны быть разными")
...
class OrderForm(forms.ModelForm):
    ...
    def __init__(self, *args, **kwargs):
        ...
        self.fields['destination'].queryset = Location.objects.filter(location_type='pickup')
...
class ItemForm(forms.ModelForm):
    ...
    def __init__(self, *args, **kwargs):
        super().__init__(*args, **kwargs)
        self.fields['current_location'].queryset = Location.objects.filter(location_type='warehouse')
...
class ShipmentForm(forms.ModelForm):
    ...
    class Meta:
        model = Shipment
        fields = ['shipment_number', 'vehicle_info', 'departure_time', 'from_location', 'to_location']
        widgets = {
    ...
    def clean(self):
        ...
        if from_location and to_location and from_location == to_location:
            raise ValidationError("Начальный и конечный пункты доставки должны быть разными")
        
        # Проверка существования маршрута
        if from_location and to_location:
            if not Route.objects.filter(from_location=from_location, to_location=to_location, active=True).exists():
    ...
    def __init__(self, *args, location=None, **kwargs):
        ...
        if location:
            self.fields['items'].queryset = Item.objects.filter(
                ...
                status__in=['created', 'at_warehouse']
            )