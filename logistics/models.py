...
class Seller(models.Model):
    ...
    contact = models.CharField("Контактная информация", max_length=200, blank=True, null=True)
...
class Location(models.Model):
    ...
    name = models.CharField("Название", max_length=100)
    location_type = models.CharField("Тип локации", max_length=20, choices=LOCATION_TYPE)
...
class Shipment(models.Model):
    ...
    items = models.ManyToManyField(Item, related_name='shipments', verbose_name="Товары")
...
class Route(models.Model):
    ...
    def clean(self):
        ...
        if self.from_location == self.to_location:
            raise ValidationError("Начальная и конечная локации не могут быть одинаковыми")
